...
@section('title', 'Optional Payments')
...
<div class="container-fluid px-4 py-3">
    ...
    <div class="d-flex align-items-center justify-content-between mb-4">
        <div>
            <h4 class="fw-bold mb-0">Optional Payments</h4>
            <small class="text-muted">Fee payments that are not compulsory — e.g. Uniform, Excursion</small>
        </div>
            ...
            <form method="GET" id="filterForm">
                <div class="row g-2 align-items-end">
                    ...
                    <div class="col-12 col-md-3">
                        <label class="form-label fw-semibold mb-1" style="font-size:12px;">Fee Type</label>
                        <select name="fee_id" id="feeSelect" class="form-select form-select-sm">
                            <option value="">All Optional Fees</option>
                            @foreach($fees as $fee)
                            <option value="{{ $fee->id }}"
                                {{ $selectedFeeId == $fee->id ? 'selected' : '' }}>
                                {{ $fee->feeType->name }} — ₦{{ number_format($fee->amount, 2) }}
                                @if($fee->schoolClass) ({{ $fee->schoolClass->name }}) @endif
                            </option>
                            @endforeach
                        </select>
                    </div>
    ...
    <div class="row g-3 mb-4">
        @php
            $cards = [
                ['label' => 'Total Records',    'value' => $summary['total'],                              'icon' => 'bi-receipt',           'color' => '#0d6efd'],
                ['label' => 'Fully Paid',        'value' => $summary['paid'],                               'icon' => 'bi-check-circle-fill', 'color' => '#198754'],
    ...
    <div class="card border-0 shadow-sm rounded-3">
        <div class="card-header bg-white border-bottom py-3 d-flex align-items-center justify-content-between">
            <h6 class="mb-0 fw-semibold" style="font-size:13px;text-transform:uppercase;letter-spacing:.05em;">
                <i class="bi bi-receipt me-2 text-primary"></i>Optional Payment Records
            </h6>
            @if($payments->total() > 0)
        ...
        <div class="card-body p-0">
                ...
                <table class="table table-hover align-middle mb-0" style="font-size:13px;">
                    <thead class="table-light">
                        <tr>
                            ...
                            <th class="py-3 pe-4">Received By</th>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-muted pe-4">
                                {{ $payment->receivedBy->fullname ?? $payment->receivedBy->name ?? '—' }}
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
            ...
            <div class="text-center py-5 text-muted">
                ...
                <p class="mb-0 fw-semibold">No optional payments found.</p>
    ...
    <div class="card border-0 shadow-sm rounded-3">
        <div class="card-body text-center py-5 text-muted">
            ...
            <p class="mb-0 fw-semibold">Select a session and term to view optional payments</p>
...
document.getElementById('sessionSelect').addEventListener('change', function () {
    ...
    feeSelect.innerHTML  = '<option value="">All Optional Fees</option>';
