                        ...
                        <tr>
                            <td class="ps-4">
                                <div class="d-flex align-items-center gap-2">
                                    ...
                                    <div>
                                        <div class="fw-semibold" style="font-size:14px;">{{ $student->short_name }}</div>
                                        <small class="text-muted d-sm-none">{{ $student->admission_number }}</small>
                                    </div>
                                </div>
                            </td>
                            <td class="d-none d-md-table-cell">
                                <span class="font-monospace" style="font-size:13px;">{{ $student->admission_number }}</span>
                            </td>
                            ...
                            <td class="pe-4 text-end">
                                <div class="d-flex gap-1 justify-content-end">
                                    ...
                                    <form action="{{ route('admin.students.toggle-status', $student) }}" method="POST">
                                        @csrf @method('PATCH')
                                        <button class="btn btn-sm {{ $student->is_active ? 'btn-outline-warning' : 'btn-outline-success' }}"
                                                title="{{ $student->is_active ? 'Deactivate' : 'Activate' }}"
                                                onclick="return confirm('{{ $student->is_active ? 'Deactivate' : 'Activate' }} {{ $student->short_name }}?')">
                                            <i class="bi {{ $student->is_active ? 'bi-person-dash' : 'bi-person-check' }}"></i>
                                        </button>
                                    </form>
                                </div>
                            </td>
