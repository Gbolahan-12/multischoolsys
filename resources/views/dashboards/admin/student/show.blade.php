                            ...
                            <form action="{{ route('admin.students.assign-class', $student) }}" method="POST">
                                ...
                                <select name="class_id" class="form-select form-select-sm mb-2" required>
                                    ...
                                    @foreach($classes as $class)
                                        <option value="{{ $class->id }}" {{ $student->currentAssignment?->class_id == $class->id ? 'selected' : '' }}>
                                            {{ $class->full_name }}
                                        </option>
                                    @endforeach
                                </select>
                ...
                <div class="card border-0 shadow-sm rounded-3">
                    <div class="card-body p-3 d-grid gap-2">
                        <form action="{{ route('admin.students.toggle-status', $student) }}" method="POST">
                            @csrf @method('PATCH')
                            <button
                                class="btn w-100 btn-sm {{ $student->is_active ? 'btn-outline-warning' : 'btn-outline-success' }}"
                    ...
                    <div class="card-body p-0">
                        <dl class="mb-0">
                            <div class="row g-0 px-4 py-3 border-bottom">
                                ...
                                <dd class="col-7 col-sm-8 mb-0 fw-semibold" style="font-size:14px;">
                                    {{ $student->full_name }}</dd>
                            </div>
                            <div class="row g-0 px-4 py-3 border-bottom">
